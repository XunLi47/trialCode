...
public class DateUtil {
    ...
    public static final String DATE_FORMAT_POINTMMDDHHMM = "MM.dd HH:mm";   
    public static final String DATE_FORMAT_POINTYYYYMMDDHHMM = "yyyy.MM.dd HH:mm";   
    ...
    public static final String DATE_TIME_FORMAT_YYYYMMDDHHMMS = "yyyy-MM-dd HH:mm:ss.S";
    ...
    public static Date parseToDate(String dateNow,String formatter) {
        ZoneId zoneId = ZoneId.systemDefault();
        LocalDate localDate=LocalDate.parse(dateNow, DateTimeFormatter.ofPattern(formatter));
    ...
    public static Date parseToDateTime(String dateNow,String formatter) {
        ZoneId zoneId = ZoneId.systemDefault();
        LocalDateTime localDate=LocalDateTime.parse(dateNow, DateTimeFormatter.ofPattern(formatter));
     ...
     * @return
     */
    public static Date millsToDateTime(Object mills) {
        Calendar instance = Calendar.getInstance();
        if(mills==null||"".equals(mills)){
            return null;
        }
        instance.setTimeInMillis(Long.valueOf(mills.toString()));
	...
	public static Date strToDate(String strTime) {  
        ...
        list.add(DATE_TIME_FORMAT_YYYYMMDDHHMMS);
     ...
     * @return 相差天数
     */
    public static boolean isOverYear(Date date1, Date date2) throws Exception{
        Calendar c = Calendar.getInstance();
        ...
        if(time1<time2){
            c.setTime(date1);
            c.add(Calendar.YEAR, 1);//加减年数
            diff=date2.getTime()-c.getTime().getTime();
        }else{
            c.setTime(date2);
            c.add(Calendar.YEAR, 1);//加减年数
            diff=date1.getTime()-c.getTime().getTime();
        }
        long l = diff / (24 * 60 * 60 * 1000);
	 ...
	 * 装换成unixtimestamp
	 */
	 public static String datetoUnix(String date){ 
		    try {
		    	DateFormat df=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss:SSS"); 
		    	Date epoch = DateUtil.parseStrToDate(date,DateUtil.DATE_TIME_FORMAT_YYYY_MM_DD_HH_MI_SS);
		    	String Date = String.valueOf(epoch.getTime());
				return Date;
			} catch (Exception e) {
	 ...
	 //蒋涛
	 public static String datetoUnix(Object date){ 
		    try {
